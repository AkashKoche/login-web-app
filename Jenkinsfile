pipeline {
    ...
    environment {
     ...
     stages {
         ...
         stage('Vulnerability Scanning') {
             parallel {
                 stage('Scan Link Service') {
                     steps {
                         catchError(buildResullt: 'SUCCESS', stageResult: 'FAILURE') {
         ...
         stage('Artifact Registry Promotion') {
             ...
             steps {
                 withCredentials([usernamePassword(credentailsId: "${env.REGISTRY_CREDENTIALS_ID}", usernameVariable: 'DOCKER_USER', password: 'DOCKER_PASS')]) {
                     sh """
                         echo "${DOCKER_PASS}" | docker login ${env.REGISTRY_URL} -u "${DOCKER_USER}" --password-stdin

                         docker tag my-web-app:latest ${env.IMAGE_LINK_SERVICE}:${env.GIT_COMMIT_SHORT}
    ...
    post {
        ...
        success {
            echo 'Pipeline completed successfully. Artifacts have beenpromoted.'
        }
        failure {
